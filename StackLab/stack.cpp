...
//PURPOSE:  checks top and returns true if "empty", i.e. if top
// eqauls -1, false otherwise. +CP
...
//PURPOSE: checks top and returns true if top is "full", i.e. if top
// is at the end of the array (MAX - 1) +CP
...
void stack::clearIt()
{
	while (!isEmpty())
	{
		// local variable temp used to help clear the stack
		int temp = 0;
		temp = el[top];
		top--;
	}
